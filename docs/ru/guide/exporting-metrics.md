...
Ntpd-rs поддерживает экспорт ключевых операционных метрик во внешний экземпляр [prometheus](https://prometheus.io/).

## Установленный из пакета

Если ntpd-rs был установлен из пакетов, распространяемых нами, конфигурация по умолчанию уже включила сокет наблюдения. Более того, эти пакеты содержат файл модуля systemd, который включает экспорт метрик с разумной конфигурацией. Это можно включить с помощью

```sh
sudo systemctl enable --now ntpd-rs-metrics
```

После включения экспорта метрик набор данных prometheus будет обслуживаться по адресу `127.0.0.1:9975/metrics` по умолчанию. Это можно настроить с помощью следующего раздела конфигурации в [ntp.toml](../man/ntp.toml.5.md#observability), чтобы предоставить доступ для Prometheus:
...
Обязательно защитите этот порт брандмауэром, чтобы только экземпляры Prometheus имели доступ.
...
## Установленный через cargo или из исходного кода

При установке через cargo или из исходного кода необходимо вручную настроить две вещи:

- Включить сокет наблюдения в конфигурации ntpd-rs.
- Настроить систему для запуска ntp-metrics-exporter как службы.

Сокет наблюдения можно включить, добавив следующее в конфигурацию:
...
Далее настройте вашу систему для запуска бинарного файла ntp-metrics-exporter как службы. Для систем на основе systemd ниже приведен пример.
